...
io.on("connection", function (socket) {
  ...
  socket.on("signup_send", async (data) => {
    ...
    try {
      ...
      database_config.sql =
        "SELECT * FROM casino_user WHERE user = ? AND email = ?";
      database_config.name = "db02";
      const existingUsers = await database(database_config, [
        data.user,
        data.email,
      ]);

      if (existingUsers && existingUsers.length === 0) {
        // Step 2: User doesn't exist, proceed with sign-up
        const uuid = crypto.randomBytes(20).toString("hex");
        const device = get_device(socket.request.headers); // Detect device type
        const timestamp = new Date().getTime().toString();
        const encryptedPass = JSON.stringify(encrypt(data.pass));
        const verificationToken = crypto.randomBytes(16).toString("hex"); // Generate verification token
        const phone = "N/A"; // Placeholder for phone, update logic as needed
        const account_type = 1; // Adjust based on application requirements
        const user_money = 100; // Default money for new users

        // Emit sign-up success (client-side update)
        const signupResponse = {
          ...
          money: user_money,
          device,
          profile_pic: null, // Update with actual logic if profile picture exists
        };
        ...
        // Step 3: Get extra data (e.g., city, country, IP)
        const extraDataResponse = await get_extra_data();
        const extraData = {
          city: extraDataResponse?.data?.city || "",
          country: extraDataResponse?.data?.country_name || "",
          ip_address: extraDataResponse?.data?.ip || "",
        };

        // Step 4: Insert user into the `casino_user` table
        database_config.sql =
          "INSERT INTO casino_user (uuid, user, email, pass, account_type, money, phone, signup, verification_token) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        database_config.name = "db03";
        const userInsertResult = await database(database_config, [
          uuid,
          data.user,
          data.email,
          encryptedPass,
        ...
        if (userInsertResult && userInsertResult.insertId) {
          ...
          // Step 5: Insert login details into the `login_user` table
          database_config.sql =
            "INSERT INTO login_user (user_id, login_date, device, ip_address, city, country) VALUES (?, ?, ?, ?, ?, ?)";
          database_config.name = "db04";
          await database(database_config, [
            userId,
            timestamp,
            device,
            extraData.ip_address,
            extraData.city,
            extraData.country,
          ]);
        } else {
          console.error("Failed to insert user into casino_user table.");
          io.to(socket.id).emit("signup_read", {
